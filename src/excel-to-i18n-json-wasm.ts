 ...
 * 
 * #[wasm_bindgen]
 * pub fn process_excel(excel_data: &[u8], options: ExcelOptions) -> JsValue {
 *   let mut results: HashMap<String, HashMap<String, String>> = HashMap::new();
 *   
 *   // Initialize result map for each language
 *   for lang in &options.support_languages {
 *     results.insert(lang.clone(), HashMap::new());
 *   }
 *   
 *   // Process Excel bytes using calamine
 *   if let Ok(mut workbook) = open_workbook_from_bytes(excel_data) {
 *     if let Some(Ok(sheet)) = workbook.worksheet_range_at(0) {
 *       for row in sheet.rows().skip(1) { // Skip header row
 *         let category = row.get(options.category_column_index)
 *                           .and_then(|c| c.get_string())
 *                           .unwrap_or_default();
 *         
 *         let key = match row.get(options.key_column_index).and_then(|c| c.get_string()) {
 *           Some(k) => k,
 *           None => continue, // Skip rows without key
 *         };
 *         
 *         // Process each language
 *         for (i, lang) in options.support_languages.iter().enumerate() {
 *           let col_index = options.value_start_column_index + i;
 ...
 * }
 * 
 * // Helper function to open workbook from bytes
 * fn open_workbook_from_bytes(data: &[u8]) -> Result<Xlsx<&[u8]>, calamine::Error> {
 *   open_workbook(data)
 * }
 */
